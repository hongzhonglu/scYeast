...
with open("data/turnover_label_filter.pkl","rb") as f:
    turnover_label_filter=torch.tensor(pickle.load(f))
with open("data/pro_interaction.pkl", 'rb') as f:
    all_embeddings = pickle.load(f)  # 形状 (5476, 1795, 200)
with open("data/turnover_label_index.pkl","rb") as f:
    turnover_label_index=torch.tensor(pickle.load(f))
with open("data/turnover_1795_index.pkl","rb") as f:
    turnover_1795_index=torch.tensor(pickle.load(f))
...
# 修改后的评估函数：多轮次K折
def evaluate_protein_regression(features, labels, n_rounds=5, n_splits=5, output_dir='turnover_kfold_results'):
    os.makedirs(output_dir, exist_ok=True)
    all_results = []
...
if __name__ == '__main__':
    ...
    # 预处理标签
    y_processed, _ = preprocess_labels(turnover_label_filter)
    
    # 提取特征
    features = extract_pooling_features(all_embeddings[:,turnover_1795_index, :])
    ...
    results = evaluate_protein_regression(
        ...
        n_rounds=5,   # 运行5轮
        n_splits=5,    # 每轮5折
        output_dir='turnover_kfold_results'
    )
