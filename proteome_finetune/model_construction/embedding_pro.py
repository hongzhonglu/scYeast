...
class AutoDiscretizationEmbedding2(nn.Module):
    def __init__(self, dim, bin_num=10, bin_alpha=1.0,mask_token_id=-1,pad_token_id=0,pad2_token_id=-2):
        super().__init__()
        ...
        self.emb_pad2 = nn.Embedding(1,self.dim)
        
        self.bin_num_idx = torch.tensor(range(self.bin_num))
        self.mask_token_id = mask_token_id
        self.pad_token_id = pad_token_id
        self.pad2_token_id = pad2_token_id
    ...
    def forward(self, x, output_weight=0):
        ...
        x_pad2_idx = (x==self.pad2_token_id).nonzero()
        ...
        pad2_token_emb = self.emb_pad2(tensor0).to(x.device).type(x.dtype)
        x[x_pad2_idx[:,0],x_pad2_idx[:,1],:] = pad2_token_emb.repeat(x_pad2_idx.shape[0],1)
    
        if output_weight:
            return x,weight
        return x,x_mask_idx,x_pad_idx,x_pad2_idx
...
if __name__ == '__main__':
    ...
    '''
    # 构造输入
    x = torch.tensor([[5, -1, 0],[3,4,-1]], dtype=torch.long)  # -1=mask, 0=pad
    model = AutoDiscretizationEmbedding2(dim=64)
    output = model(x)
    ss = output.sum()
    ss.backward()
    
    #检查梯度
    int(model.mlp.weight.grad)       # 部分位置梯度为0（被替换的位置无梯度）
    int(model.emb_mask.weight.grad)  # 非零（替换嵌入的梯度正常更新）