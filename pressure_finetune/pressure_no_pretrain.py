...
class AdditionalNetwork(nn.Module):
    def __init__(self, dropout_prob=0.1):
        ...
        self.conv1d2 = nn.Conv1d(in_channels=512, out_channels=4, kernel_size=1)
        self.gelu = nn.GELU()
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        optimizer = optim.AdamW(addition_net.parameters(), lr=0.0005, weight_decay=0.01)
        max_epoch = 6
        best_val_acc = 0.0
        ...
        for epoch in range(max_epoch):
            ...
            if val_acc >= best_val_acc:
                ...
                best_model_state = addition_net.state_dict()
        addition_net.load_state_dict(best_model_state)
        torch.save(best_model_state, f'pressure_no_pretrain_best_model_run_{i+1}.pth')
    ...
    # 保存为numpy文件
    np.save('pressure_no_pretrain_test_predictions.npy', all_predictions)
    np.save('pressure_no_pretrain_test_true_labels.npy', all_true_labels)
...
all_predictions = np.load('pressure_no_pretrain_test_predictions.npy')
all_true_labels = np.load('pressure_no_pretrain_test_true_labels.npy')
